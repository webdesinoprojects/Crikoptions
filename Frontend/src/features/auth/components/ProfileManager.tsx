...
import { AlertCircle, CheckCircle, Loader2, Camera, Calendar, TrendingUp, Trophy, Target, Shield, Info, Bell, Wallet, Mail, Phone, Award } from "lucide-react";
    ...
    <div className="max-w-[1400px] mx-auto flex flex-col gap-6 font-sans select-none pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-[#d4af37]/25 bg-gradient-to-br from-[#1a1508] via-[#0c0a06] to-[#050505] p-6 shadow-[0_10px_40px_rgba(212,175,55,0.06)]">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(212,175,55,0.12),transparent_42%)]" />
            <div className="absolute top-0 left-0 h-px w-full bg-gradient-to-r from-transparent via-[#d4af37]/70 to-transparent" />

            <div className="relative z-10 flex items-center justify-between">
              <div className="flex items-center gap-2 text-[#d4af37] text-[11px] font-semibold tracking-[0.18em] uppercase">
                <Trophy className="w-3.5 h-3.5" />
                Trader Card
              </div>
              <span className="rounded-full border border-[#d4af37]/25 bg-[#d4af37]/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[#d4af37]">
                {user.tier || "Standard"}
              </span>
            </div>
            ...
            <div className="relative z-10 mt-6 flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-[0.16em] text-white/35">Issued to</p>
                <h3 className="mt-1 truncate text-xl font-semibold tracking-tight text-white">{name}</h3>
                <p className="mt-1 font-data-tabular text-[11px] text-white/40">
                  ID {memberId} · Since {joinDate}
                </p>
              </div>
              {featuredBadge ? (
                <AcademyBadge badge={featuredBadge} unlocked size="md" />
              ) : (
            ...
            <div className="relative z-10 mt-5 flex items-center justify-between border-t border-white/8 pt-4">
              <p className="text-[11px] text-white/35">Simulation desk · Paper trading</p>
              <Link href="/challenges" className="text-[11px] font-medium text-[#d4af37] hover:text-[#f0d78c]">
                View challenges
              </Link>
            </div>
          </div>
