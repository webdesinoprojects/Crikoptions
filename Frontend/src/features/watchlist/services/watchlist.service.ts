...
export const watchlistService = {
  fetchWatchlist: async (userId: string = "0"): Promise<Watchlist> => {
  ...
  addWatchlist: async (marketId: string): Promise<unknown> => {
    const response = await apiClient.post("/v1/watchlist", { marketId });
    return response.data;
  },

  removeWatchlist: async (marketId: string): Promise<unknown> => {
