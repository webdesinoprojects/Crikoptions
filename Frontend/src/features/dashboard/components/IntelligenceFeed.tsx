...
import { Activity, AlertTriangle, BrainCircuit } from "lucide-react";
...
export function IntelligenceFeed() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "ALERT":
        return <AlertTriangle className="w-4 h-4 text-bear-red" />;
      case "CATALYST":
        return <Activity className="w-4 h-4 text-bull-green" />;
      default:
        return null;
    }
  };
      ...
      <CardContent className="space-y-3">
        {signals.length > 0 ? (
          ...
          <div className="py-8 text-center text-xs text-on-surface-variant">
            No backend intelligence signals
          </div>
        )}
      </CardContent>
    </Card>
